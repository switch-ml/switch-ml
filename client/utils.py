...
def load_partition(idx: int):
    ...
    assert idx in range(20)
    trainset, testset, num_examples = load_data()
    n_train = int(num_examples["trainset"] / 60)
    n_test = int(num_examples["testset"] / 60)

    train_parition = torch.utils.data.Subset(trainset, range(idx * n_train, (idx + 1) * n_train))
    test_parition = torch.utils.data.Subset(testset, range(idx * n_test, (idx + 1) * n_test))

    print("idx :",idx,"Original train&test len:", len(trainset),len(testset),"Partition train&test len:",len(train_parition),len(test_parition))
...
def test(net, testloader, device,steps: int = None):
